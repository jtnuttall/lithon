...
decodeProfile :: LBS.ByteString -> Either Text Profile
decodeProfile = first T.pack . eitherDecodeJSONViaCodec
...
instance HasCodec Profile where
  codec =
    object "Profile"
      $ Profile
      <$> requiredField "name" "profile identity, echoed in reports"
      .= (.name)
      <*> optionalFieldOrNull "description" "human summary"
      .= (.description)
      <*> optionalFieldWithDefault
        "registry"
      ...
      .= (.registry)
      <*> requiredField "core" "baseline and optional core versions"
      .= (.core)
      <*> optionalFieldWithDefault "platforms" [] "registry platform names in scope"
      .= (.platforms)
      <*> optionalFieldWithDefault "extensions" [] "curated extension list"
      .= (.extensions)
      <*> optionalFieldWithDefault "exclude" [] "hard bans; a required ban is an error"
      .= (.exclude)
      <*> optionalFieldWithDefault "policy" defaultPolicy "curation policy knobs"
      .= (.policy)
...
instance HasCodec RegistryPin where
  codec =
    object "RegistryPin"
      $ RegistryPin
      <$> optionalFieldOrNull "headerVersion" "expected VK_HEADER_VERSION"
      .= (.headerVersion)
...
instance HasCodec ProfileCore where
  codec =
    object "ProfileCore"
      $ normalize
      <$> requiredFieldWith "baseline" coreVersionCodec "minimum core version"
      .= (.baseline)
      <*> optionalFieldOrNullWith "max" coreVersionCodec "highest optional core version"
      .= (\c -> Just c.maxCore)
   where
    normalize baseline maxCore = ProfileCore{baseline, maxCore = fromMaybe baseline maxCore}
   ...
   where
    ...
    extensionObject =
      object "ProfileExtension"
        $ ProfileExtension
        <$> requiredField "name" "registry extension name"
        .= (.name)
        <*> optionalFieldOrNull "rationale" "why this is in the curated set"
        .= (.rationale)
...
instance HasCodec LegacyCorePolicy where
  codec =
    object "LegacyCorePolicy"
      $ LegacyCorePolicy
      <$> requiredField "mode" "annotate or exclude"
      .= (.mode)
      <*> optionalFieldWithDefault "categories" [] "deprecate categories; empty = all"
      .= (.categories)
...
instance HasCodec ProfilePolicy where
  codec =
    object "ProfilePolicy"
      $ ProfilePolicy
      <$> optionalFieldWithDefault "dependencies" DepsAuto "auto-include vs strict"
      .= (.dependencies)
      <*> optionalFieldWithDefault
      ...
      .= (.includePromotedToOptionalCores)
      <*> optionalFieldWithDefault "allowProvisional" [] "provisional allowlist"
      .= (.allowProvisional)
      <*> optionalFieldWithDefault "denySpecialUse" [] "rejected specialuse tokens"
      .= (.denySpecialUse)
